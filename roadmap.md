...
#### 9\. Add Organizer Participation Toggle
...
#### 10\. Add Public Attendee List Option
...
#### 11\. Display Event Capacity on RSVP Page
...
#### 12\. Redesign Event Capacity Progress Bar
...
-   "Plus One" / Guest Count Support: Enhance the RSVP experience by allowing a single invitee to RSVP for more than one person. The Event model would be updated to allow organizers to specify if an invitee can bring guests. The capacity calculation logic would be modified to sum the total number of attending guests, not just confirmed RSVPs. The public RSVP page would be updated to conditionally show an input for the guest to specify their party size.